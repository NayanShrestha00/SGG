...
const ManagerHeader = () => {
  ...
  const handleLogout = async () => {
    try {
      const response = await axios.post(AdminManagerSummaryApi.ManagerLogout.url, {}, {
  ...
  return (
    <aside className="bg-[#1c252e] w-[20%] p-6 flex flex-col h-screen fixed z-50 text-[#707a7f]">
      ...
      <nav className="space-y-2 flex-grow">
        {[
          { path: "/manager/dashboard", label: "Home" },
          { path: "/manager/createpost", label: "Create Post" },
          { path: "/manager/totalpost", label: "Total Post" },
          { path: "/manager/enquaryforms", label: "Enquary Forms" },
          { path: "/manager/sellerforms", label: "Seller Forms" },
          { path: "/manager/buyerforms", label: "Buyer Forms" },
        ].map((item) => (
          <NavLink
            key={item.path}
            to={item.path}
